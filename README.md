...
- 29项标准库测试、端到端烟雾测试和GitHub Actions持续集成。
...
- 免费入口用BaoStock提供的历史ST/交易状态，但没有严格历史行业和时点市值，默认不做
  行业/市值中性化。
- 首次窗口用当前中证500名单回填历史，存在幸存者偏差；项目从首次运行起保存每日名单，
  不能把首份结果表述为“无幸存者偏差的历史指数回测”。
- 回测未实现完整历史指数成分、历史申万行业、公司行动现金流、成交量容量模型、
  风险模型和实盘订单管理。
...
第一次只取最近800个自然日，不下载全部历史；以后每次只向BaoStock请求各股票本地最后
日期之后的数据。输出在`reports/generated/daily_factor_lab/`：
